...
(function($) {
    ...
    $.fn.flexbox.defaults = {
        ...
        showArrow: false, // set to false to simulate google suggest
        arrowQuery: '', // the query to run when the arrow is clicked
        onSelect: false, // function to run when a result is selected
        maxCacheBytes: 32768, // in bytes, 0 means caching is disabled
        resultTemplate: '{id}-{name}', // html template for each row (put json properties in curly braces)
        ...
        paging: {
            ...
            summaryTemplate: '{total}条' // can use {page} and {pages} as well
        }
    };

    $.fn.setValue = function(idd,name) {
        var id = '#' + this.attr('id');
		$(id + '_hidden').val(idd) ; 
		 
        $(id + '_input').val(name).removeClass('watermark');
    };
