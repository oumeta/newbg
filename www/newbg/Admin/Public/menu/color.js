...
var color_id = 1;
var color_bk = color_htm = '';
color_htm += '<table cellpadding="0" cellspacing="0" bgcolor="#2875B9" width="160">';
color_htm += '<tr><td width="2" height="20"> </td>';
color_htm += '<td><input type="text" style="width:60px;height:12px;border:#A0A0A0 1px solid;" value="" id="color_viewview" onblur="color_select(this.value);" onkeyup="color_view(this.value);" ondblclick="this.value=\'\';" title="可直接填写颜色值，双击鼠标左键清空"/></td>';
...
function color_show(id, color, obj) {
	Eh();
	if($('destoon_color') == null) {
		var destoon_color_div = document.createElement("div");
		with(destoon_color_div.style) {
			zIndex = 9999;
			position = 'absolute';
			display = 'none';
			width = '160px';
	...
	$('destoon_color').style.left = (obj.offsetLeft + leftpos) + 'px';
	$('destoon_color').style.top = (obj.offsetTop + toppos + 20) + 'px';
	$('destoon_color').innerHTML = color_htm;
	color_id = id;
	color_bk = color;
	$('destoon_color').style.display = '';
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {
	$('destoon_color').style.display = 'none';
	Es();
}
function color_close() {
	color_hide();
	$('color_img_'+color_id).style.backgroundColor = color_bk;
}
function color_select(color) {
	$('color_input_'+color_id).value = color;
	$('color_img_'+color_id).style.backgroundColor = color;
	color_hide();
}
function color_setup(color) {
	...
	colors_select += '</table>';
	$('destoon_color_show').innerHTML = colors_select;
}
function color_view(color) {
	try {
		$('color_viewview').value = color;
		$('color_viewview').style.color = color;
		$('color_img_'+color_id).style.backgroundColor = color;
	} catch(e) {}
}