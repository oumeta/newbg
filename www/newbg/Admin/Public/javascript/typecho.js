...
Typecho.Table = {
    ...
    bindButtons: function() {
        $(document).getElements(".typecho-table-select-all").addEvent("click",
        function() {
        ...
        $(document).getElements(".typecho-table-select-none").addEvent("click",
        function() {
            Typecho.Table.table.getElements(Typecho.Table.table._childTag + " input[type=checkbox]").each(function(a) {
                if (a.getProperty("checked")) {
                    a.fireEvent("click")
        ...
        $(document).getElements(".typecho-table-select-submit").addEvent("click",
        function() {
            var b = this.get("lang");
        ...
        if (this.duration > this.options.time) {
            ...
            new Request.JSON({
                ...
                onSuccess: (function(b) {
                    if (b.success) {
                        $(this.options.form).getElement("input[name=cid]").set("value", b.cid)
                    }
                    if (null != this.options.messageElement) {
                        $(this.options.messageElement).set("html", b.message);
                        $(this.options.messageElement).highlight("#ff0000")
                    }
                }).bind(this)
            }).send($(this.options.form).toQueryString() + "&" + Hash.toQueryString(a))
        }
    }
});
Typecho.textarea = new Class({
    ...
    initialize: function(c, b) {
        this.textarea = $(document).getElement(c);
        this.range = null;
        this.setOptions(b);
    ...
    makeResizeAble: function() {
        this.resizeOffset = this.textarea.getStyle("height") ? this.textarea.getSize().y - parseInt(this.textarea.getStyle("height")) : 0;
        this.resizeMouseY = 0;
        this.lastMouseY = 0;
    ...
    resizeListener: function() {
        if (this.isResizePressed) {
            ...
            var b = this.textarea.getSize().y - this.resizeOffset + a;
    ...
    resizeMouseUp: function(b) {
        if (this.isResizePressed) {
            this.isResizePressed = false;
            var a = this.textarea.getSize().y - this.resizeOffset;
            new Request({
                method: "post",
    ...
    setContent: function(b, e) {
        ...
        this.textarea.set("value", d.substr(0, a.start) + b + f + e + d.substr(a.end)); (function() {
...
Typecho.autoComplete = function(k, i) {
    ...
    m = false,
    p = 0,
    o = $(document).getElement(k).setProperty("autocomplete", "off");
    var e = function() {
        var s = 0,
        t = o.get("value");
        q = [];
        if (t.length > 0) {
            t.split(g).each(function(x, v) {
                var u = s + x.length,
                w = 0,
                y = 0;
                x = x.replace(/(\s*)(.*)(\s*)/,
    ...
    var r = function(s) {
        ...
        var t = s.length > 0 ? i.filter(function(v) {
            return (0 == v.toLowerCase().indexOf(s.toLowerCase()) && !u.contains(v))
        }) : [];
        return u.extend(t)
    };
    var a = function(t, u) {
    ...
    var n = function(s, t) {
        ...
        var u = new Element("ul", {
            ...
            styles: {
                width: o.getSize().x - 2,
                left: o.getPosition().x,
                top: o.getPosition().y + o.getSize().y
            }
        });
        t.each(function(w, v) {
            u.grab(new Element("li", {
                ...
                events: {
                    ...
                    click: function() {
                        ...
                        var y = p > 0 ? p: o.getSelectedRange().start,
                        x = f(y);
        ...
        $(document).getElement("body").grab(u)
    };
    var d = function() {
        var s = $(document).getElement(".autocompleter-choices");
        if (s) {
            s.destroy();
            m = false
        }
    ...
    o.addEvents({
        mouseup: function(u) {
            var t = o.getSelectedRange().start,
            s = f(t);
            d();
            a(t, s);
            this.fireEvent("keyup", u);
            p = o.getSelectedRange().end;
            u.stop();
            return false
        },
        blur: function() {
        ...
        keydown: function(u) {
            ...
            case "up":
                if (b.length > 0 && h >= 0) {
                    if (h < b.length) {
                        $(document).getElement(".autocompleter-choices li[rel=" + h + "]").removeClass("autocompleter-selected")
                    }
                    if (h > 0) {
                        h--
                    } else {
                    ...
                    $(document).getElement(".autocompleter-choices li[rel=" + h + "]").addClass("autocompleter-selected");
