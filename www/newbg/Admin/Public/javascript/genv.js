...
Genv.ajax.accept = '*\/*';
Genv.ajax.prototype.open = function(params) {
	...
	Genv.extend(this, {
		...
		contentType: Genv.ajax.type[params.contentType] || Genv.ajax.type['default'],
		dataType: Genv.ajax.type[params.dataType] ? Genv.ajax.type[params.dataType] + ', *\/*' : Genv.ajax.accept,
...
Genv.get = function(params) { 
	Genv.extend(params, {method: 'GET'})
	 
	new Genv.ajax().open(params);
	return this;
};
Genv.post = function(params) {
	new Genv.ajax().open(Genv.extend(params, {method: 'POST'}));
	return this;
};

Genv.getJSON = function(params, callback, error) {
	new Genv.ajax().open(Genv.extend(params, {dataType: 'json', success: function(response) {
		try {
...
Genv.Element.implement({
	noselect:function(b){
	   G.each(this._els,
	   function(el){
		 $(el).attr('unselectable', 'on').css('MozUserSelect', 'none').on('selectstart', function() { return false; });  
	   })
	}
});
