...
@import '<?=$_public?>newcss/date_input.css';
 ...
 <div style="visibility: inherit; width: 750px;" class="yui-module yui-overlay yui-panel" id="toolBoxHolder">
    ...
    <div class="bd">
    <form id="myform" action="<? echo U('save')?>" method=post>
        <fieldset>
            <legend>基本信息</legend>
            <div >
				<label>选择对像:
					<select name=comcate id=comcate onChange=searchAct(this.options[this.selectedIndex].value)>
						<option value=''></option>
						 <option value=0 <? if($rs['comcate']==0&&!is_null($rs['comcate'])) { ?>selected<? } ?>>客户</option>
						 <option value=1 <? if($rs['comcate']==1) { ?>selected<? } ?>>报关</option>
						 <option value=2 <? if($rs['comcate']==2) { ?>selected<? } ?>>托车</option>
						 <option value=4 <? if($rs['comcate']==4) { ?>selected<? } ?>>核销单</option>
			 ...
			 <div >
				<label>选择类型:
					<select name=cate id=cate>
						 <option value=0 <? if($rs['cate']==0) { ?>selected<? } ?>>收款</option>
						 <option value=1 <? if($rs['cate']==1) { ?>selected<? } ?>>付款</option>
						 <option value=2 <? if($rs['cate']==2) { ?>selected<? } ?>>回扣</option>
			...
			<div >
				<label>选择银行:
					<select name=bankid id=bankid>
					 ...
					 <option value=<?=$key?> <? if($rs['bankid']==$key) { ?>selected<? } ?>><?=$v?></option>
					 <?}?>
					</select>
				</label>
			</div>
