 ...
 * 
 * This is the file-based adapter for [Genv_Cache:HomePage Genv_Cache].
 * In general, you never need to instantiate it yourself; instead,
 * use Genv_Cache as the frontend for it and specify
 * 'Genv_Cache_File' as the 'adapter' config key value.
 * 
 ...
 * objects in the cache ... just like Genv_Cache_Memcache.
 * 
 * @category Genv
 * 
 * @package Genv_Cache
 ...
 * 
 */
class Genv_Cache_Adapter_File extends Genv_Cache_Adapter
{
    /**
     * 
     ...
     *   process. Default is '/Genv_Cache_File' in the system temporary
     ...
     * 
     */
    protected $_Genv_Cache_Adapter_File = array(
    ...
    {
        parent::_preConfig();
        $tmp = Genv_Dir::tmp('/Genv_Cache_File/');
        $this->_Genv_Cache_Adapter_File['path'] = $tmp;
    }
    ...
    protected function _postConstruct()
    {
        ...
        $this->_path = Genv_Dir::fix($this->_config['path'] . '/' . $this->_prefix);
    ...
    public function entry($key)
    {
        if ($this->_config['hash']) {
            return $this->_path . $key;//hash('md5', $key);
        } else {
