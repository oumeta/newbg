...
uri ÖØÐ´;
*/
class Genv_Uri_Rewrite extends Genv_Base{
  
    protected $_Genv_Uri_Rewrite = array(
        'rewrite' => array(),
        'replace' => array(),
    );
     ...
     * rewritten path.
     * 
     * @param string|Genv_Uri $spec The original URI path.
     * 
    ...
    public function match($spec)
    {
        ...
        if ($spec instanceof Genv_Uri_Action) {
			//echo $spec->getFrontPath();
            $oldpath = trim($spec->getFrontPath(), '/');
        } elseif ($spec instanceof Genv_Uri) {
