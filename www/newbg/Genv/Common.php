...
class Genv_Common
{
    ...
    public static function init()
    {
        ...
        function __autoLoad($className)
        {
             ...
             * 如果你需要判断一个类能否被加载, 请使用 Genv_Common::isAvailableClass 方法
             */
        ...
        /** 设置异常截获函数 */
        set_exception_handler(array('Genv_Common', 'exceptionHandle'));
    }
    ...
    public static function error($exception)
    {
        ...
        require_once 'Genv/Response.php';
        $charset = self::$charset;

        if ($isException && $exception instanceof Genv_Db_Exception) {
            $code = 500;
            @error_log($message);
            ...
            if ($exception instanceof Genv_Db_Adapter_Exception) {
                $code = 503;
                $message = 'Error establishing a database connection';
            } else if ($exception instanceof Genv_Db_Query_Exception) {
        ...
        if (is_numeric($code) && $code > 200) {
            require_once 'Genv/Response.php';
            Genv_Response::setStatus($code);
        }

        $message = nl2br($message);

        if (defined('__Genv_EXCEPTION_FILE__')) {
            require_once __Genv_EXCEPTION_FILE__;
        } else {
            echo
<<<EOF
...
EOF;
    ...
    public static function safeUrl($url)
    {
        ...
        /** 过滤解析串 */
        $params = array_map(array('Genv_Common', '__removeUrlXss'), $params);
