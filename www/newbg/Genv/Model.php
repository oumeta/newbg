...
class Genv_Model extends Genv_Base{
	...
	*/
	 ...
	 public function __call($method,$args) {
        ...
        if(in_array($method,array('selectfrom','where','orWhere','order','setpage','page','limit','group','bind','countPages','fetch','leftJoin','innerJoin'),true)) {
	...
	//	dump($data);
		...
		return $this->db->insert($options['from'],$data,$options);		
	...
	public function replace($data,$options=array()) {
		...
		$sql=$this->parseSql($options);
		$sql->setAttribute('action','REPLACE');
		$sql=$sql->rows($data,$options);			
	...
	//	dump($data);
        ...
        $options['from'] =ereg_replace("^table.",$this->_prefix,$options['from'] );
		// dump($data);	
		return $this->db->update($options['from'],$data,$options);		
		 	 
      //  return $this->db->query($sql);
    }
	
	//批量添加
	function addall( $datas,$options=array()) {
		
			$dt=array();
			$rs=array();
			foreach($datas as $key=>$data){			 
			 $rs[]=$this->add($data,$options);
			}         
			return $rs;
	}
	 ...
	 public function delete($options=array()){
        ...
        $options['from'] =ereg_replace("^table.",$this->_prefix,$options['from'] );
		//$sql=$this->parseSql($options)->delete();
        return $this->db->delete($this->options['from'],$where);
    }
