 ...
 * 
 * {{code: php
 *     $select = Genv::factory('Genv_Db_Query');
 *     
 ...
 * 
 */
class Genv_Db_Query extends Genv_Base
{
    /**
     * 
     * A constant so we can find "ignored" params, to avoid func_num_args().
     * 
     * The md5() value of 'Genv_Db_Query::IGNORE', so it should be unique.
     * 
     ...
     * 
     * @config dependency sql A Genv_Db dependency object.
     * 
     ...
     * 
     */
    protected $_Genv_Db_Query = array(
        'db'    => 'db',
		'model'    => 'Dev',
        'paging' => 10,
    );
    ...
    protected function _postConstruct()
    {
        parent::_postConstruct();
		 ...
		 // dump($this->_config);
		 $config=Genv_Config::get('Genv_Db',$this->_config['model']);
		// dump($config);
		 Genv_Config::set('Genv_Db','adapter','Genv_Db_Adapter_'.$config['type']);
	
        // connect to the database with dependency injection
       $this->_db = Genv::dependency('Genv_Db', $this->_config['db']);
        
        // set up defaults
        $this->setpage($this->_config['paging']);
    }
     ...
     * 
     * @return Genv_Db_Query
     * 
     */
    public function setpage($rows)
    {
     ...
     * 
     * @param string|object $spec If a Genv_Sql_Model object, the table
	...
	/*×Ô¶¨Òå*/
	 public function selectfrom($spec, $cols = null){
        // get the table name and columns from the specifcation
        list($name, $cols) = $this->_nameCols($spec, $cols);
     ...
     * 
     * @param string|Genv_Db_Query $spec If a Genv_Db_Query
     ...
     * as they are, but converts Genv_Db_Query objects to strings after
     * merging bind values.
     * 
     * @param string|Genv_Db_Query $spec The select to prepare as a 
     * sub-select.
     * 
     * @return string
     * 
     ...
     * 
     */
    public function where($cond, $val = Genv_Db_Query::IGNORE)
    {
        if (empty($cond)) {
            return $this;
        }
        
        $cond = $this->_db->quoteNamesIn($cond);
        
        if ($val !== Genv_Db_Query::IGNORE) {
     ...
     * 
     */
    public function orWhere($cond, $val = Genv_Db_Query::IGNORE)
    {
        if (empty($cond)) {
            return $this;
        }
    ...
    public function multiWhere($list, $op = 'AND')
    {
        ...
        foreach ((array) $list as $key => $val) {
            if (is_int($key)) {
                ...
                $this->_multiWhere($val, Genv_Db_Query::IGNORE, $op);
            } else {
     ...
     * 
     */
    public function having($cond, $val = Genv_Db_Query::IGNORE)
    {
        if (empty($cond)) {
            return $this;
        }
     ...
     * 
     */
    public function orHaving($cond, $val = Genv_Db_Query::IGNORE)
    {
        if (empty($cond)) {
            return $this;
        }
    ...
    public function multiHaving($list, $op = 'AND')
    {
        ...
        foreach ((array) $list as $key => $val) {
            if (is_int($key)) {
                // integer key means a literal condition
                // and no value to be quoted into it
                $this->_multiHaving($val, Genv_Db_Query::IGNORE, $op);
            } else {
                // string $key means the key is a condition,
                // and the $val should be quoted into it.
    ...
    public function order($spec)
    {
        ...
        $spec = $this->_db->quoteNamesIn($spec);
    ...
    public function fetch($type = 'sql')
    {
        ...
        return $this->_db->$fetch($parts, $this->_bind);
    }
     ...
     * 
     * @param Genv_Db_Query $inner The inner subselect to use.
     * 
    ...
    protected function _countSubSelect($inner, $col)
    {
        ...
        $outer = clone($this);
        $outer->clear();
        $outer->fromSelect($inner, $alias, "COUNT($distinct$col) as genvid");
     ...
     * 
     * @see Genv_Sql_Adapter::quoteInto()
     * 
     */
    public function quoteInto($txt, $val)
    {
        return $this->_db->quoteInto($txt, $val);
    }
     ...
     * quoted values.
     * 
     * @see Genv_Sql_Adapter::quoteMulti()
     * 
     */
    public function quoteMulti($list, $sep = null)
    {
        return $this->_db->quoteMulti($list, $sep);
    }
     ...
     * "LEFT", "INNER", etc.
     * 
     * @param string|Genv_Sql_Model $spec If a Genv_Sql_Model
     ...
     * 
     * @param string|Genv_Sql_Model $spec The specification for the table
    ...
    protected function _nameCols($spec, $cols)
    {
        ...
        if ($spec instanceof Genv_Sql_Model) {
    ...
    {
        if ($name == $orig) {
            $parts['from'][] = $this->_db->quoteName($name);
        } else {
            $parts['from'][] = $this->_db->quoteName($orig)
                             . ' '
    ...
    protected function _buildJoin(&$parts, $name, $orig, $join, $cond)
    {
        ...
        if ($name == $orig) {
            $tmp['name'] = $this->_db->quoteName($name);
        } elseif ($orig[0] == '(') {
            $tmp['name'] = $orig
                         . ' '
                         . $this->_db->quoteName($name);
        } else {
            $tmp['name'] = $this->_db->quoteName($orig)
                         . ' '
                         . $this->_db->quoteName($name);
        }
    ...
    {
        $parts['from'][] = "($orig) " . $this->_db->quoteName($name);
    }
