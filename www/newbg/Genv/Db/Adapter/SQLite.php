...
/** 数据库适配器接口 */
require_once 'Genv/Db/Adapter.php';
 ...
 * @package Db
 */
class Genv_Db_Adapter_SQLite implements Genv_Db_Adapter
{
    /**
     * 数据库标示
     *
     ...
     * 数据库连接函数
     *
     * @param Genv_Config $config 数据库配置
     * @throws Genv_Db_Exception
     * @return resource
     */
    public function connect(Genv_Config $config)
    {
        ...
        /** 数据库异常 */
        require_once 'Genv/Db/Adapter/Exception.php';
        throw new Genv_Db_Adapter_Exception($error);
    }
     ...
     * @return resource
     */
    public function query($query, $handle, $op = Genv_Db::READ, $action = NULL)
    {
        if ($resource = @sqlite_query($query instanceof Genv_Db_Query ? $query->__toString() : $query, $handle)) {
            return $resource;
        }

        /** 数据库异常 */
        require_once 'Genv/Db/Query/Exception.php';
        $errorCode = sqlite_last_error($this->_dbHandle);
        throw new Genv_Db_Query_Exception(sqlite_error_string($errorCode), $errorCode);
    }
