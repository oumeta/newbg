...
class Genv_Db_Adapter_Mysql extends Genv_Db_Adapter{
    ...
    public  function connect($name='dev'){
        ...
        $this->dlink=$this->dbpool[$name]=mysqli_connect( $cfg['host']. ':' .$cfg['port'],$cfg['user'], $cfg['pass']);
        ...
        if ( $this->has_cap( 'collation' ) && !empty( $cfg['charset']) ) {
            ...
            if ( function_exists( 'mysql_set_charset' ) ) {
                mysqli_set_charset( $cfg['charset'], $this->dlink );
...
//					$query .= $this->prepare( ' COLLATE %s', $this->collate );
        ...
        mysqli_query($this->dlink,"set names 'utf8'");
    ...
    public   function selectdb($db){
        if ( !mysqli_select_db($this->dlink,$db)){
    ...
    public  function query($query){
        ...
        $this->result = mysqli_query($this->dlink,$query);
        $this->num_queries++;
        $this->logs($query);
        ...
        {
            $this->rows_affected = mysqli_affected_rows($this->dlink);

            // Take note of the insert_id
            if ( preg_match("/^(insert|replace)\s+/i",$query) )
            {
                $this->rows_affected = mysqli_insert_id($this->dlink);
            }
            //dump( $this->rows_affected);
    ...
    function db_version() {
        // dump($this->dlink);
        return preg_replace( '/[^0-9.].*/', '', mysqli_get_server_info( $this->dlink ) );
    }
