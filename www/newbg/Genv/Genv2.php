...
class Genv{
    ...
    public static function client($name){
		...
		$config['Genv']['cachetemp'] =$config['Genv']['appdir']."/Temp/~temp.php";

		$config['Genv']['cacherun'] =$config['Genv']['appdir']."/Temp/~run.php";
		...
		if(!file_exists($config['Genv']['cachetemp'])){
			$fun="php_strip_whitespace";
			 $fun="file_get_contents";
			$l=$fun($config['Genv']['appdir']."/Conf/Webconf.php");
			$list = array(           
				'Function' ,
				'Base',
				'Class',
    ...
    public static function start($config = array()){
	    ...
	    if(file_exists($config['Genv']['cacherun'])){
